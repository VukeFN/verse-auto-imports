...
- Feedback surfaces: success and background outcomes go to the status bar (`setStatusBarMessage`, 3000 ms); a notification toast only where an action button is attached or the message answers an explicit user request; a modal dialog never carries a read-only report.
