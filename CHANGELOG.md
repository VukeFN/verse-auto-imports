...
- **Commented-Out Imports Are No Longer Treated As Live**: a `using` statement inside a `<# ... #>` block comment is now ignored by every import operation. The scanner tracked only indentation, so a commented-out import block — which sits at column 0 like a live one — was read as real, with four consequences: a new import was written *into* the comment while the status bar reported success and the compiler error stayed put; an import that was needed but present-only-as-a-comment was judged already there, so nothing happened at all; **Optimize Imports** hoisted a deliberately disabled import back into force and left the comment as an empty husk; and saving inserted a stray blank line inside the comment. Nested block comments and the indented `<#>` comment form are handled, and a `<#` inside a `#` line comment no longer opens one. Single-line `#` comments were never affected ([#127])
...
- **Trailing Comments No Longer Corrupt Rewritten Imports**: an import written in dot syntax with a trailing comment, such as `using. /Verse.org/Simulation # note`, is no longer rewritten as `using { /Verse.org/Simulation # note }`, where the `#` opens a comment that swallows the closing brace and breaks the statement. The path capture ran to end of line with no comment terminator, so the comment became part of the path; a trailing `#` line comment or `<# #>` block comment is now stripped before the path is used, across all three `using` styles and in path conversion and scanning alike. A trailing comment on an import that gets rewritten is still dropped rather than carried through, which matches the existing braced-syntax behavior ([#120])
