...
- Feedback surfaces: success and background outcomes go to the status bar (`setStatusBarMessage`, 3000 ms); a notification toast only where an action button is attached, or where the message is itself the command's whole product (a requested report) — a routine success acknowledgment never qualifies; a modal dialog never carries a read-only report.
